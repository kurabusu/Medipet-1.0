...
public class servletLogin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(ValidaLogin(user, pass)){
                for(int x=0;x< ListarDatos().size();x++){
                    if(ListarDatos().get(x).getEmail().equals(user)){
                        if(ListarDatos().get(x).getPerfil()==1){
                ...
                out.println("</head>");
                out.println("<body>");
                out.println("<h1>Ingrese Credenciales Válidas</h1>");
                out.println("<a href= './loginUsuario.jsp'>Volver Atrás</a>");
                out.println("</body>");
                out.println("</html>");
            }
    ...
    public boolean ValidaLogin(String user, String pass){
        ...
        boolean valido;
        valido = ctrl.ValidaLogin(user, pass);
