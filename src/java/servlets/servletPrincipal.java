...
public class servletPrincipal extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ControlAdmin ctrlAd = new ControlAdmin();
            
            RequestDispatcher dispatcher;
            
            
            if(!ctrlAd.validaAdmin()){
                HttpSession sesion = request.getSession(true);
                sesion.setAttribute("s_perfiles", PerfilListar());
                
                dispatcher = request.getRequestDispatcher("/crearAdmin.jsp");
                dispatcher.forward(request, response);
                
            }else{
                HttpSession sesion = request.getSession(true);
                sesion.setAttribute("s_user", ListarDatos());
                
                dispatcher = request.getRequestDispatcher("/loginUsuario.jsp");
                dispatcher.forward(request, response);
            }
